...
function AdminProductManagement() {
  ...
  const handleNewProductChange = (e) => {
    ...
    if (name === "image") {
      [span_6](start_span)setImageFile(files[0]);[span_6](end_span)
      [span_7](start_span)setNewProduct((prev) => ({ ...prev, image: files[0] }));[span_7](end_span)
    } else {
      [span_8](start_span)setNewProduct((prev) => ({ ...prev, [name]: value }));[span_8](end_span)
    }
  };

  const handleAddProduct = async (e) => {
    [span_9](start_span)e.preventDefault();[span_9](end_span)
    try {
      [span_10](start_span)let imageUrl = null;[span_10](end_span)
      if (imageFile) {
        [span_11](start_span)const asset = await client.assets.upload("image", imageFile);[span_11](end_span)
        [span_12](start_span)imageUrl = asset._id;[span_12](end_span)
      }
      ...
      const productDoc = {
        ...
        ...(imageUrl && {
          [span_13](start_span)image: { _type: "image", asset: { _ref: imageUrl } },[span_13](end_span)
        }),
      };

      await client.create(productDoc);
      [span_14](start_span)alert("Product added successfully!");[span_14](end_span)
      setNewProduct({
        title: "",
        description: "",
        sku: "",
      ...
      [span_16](start_span)setImageFile(null);[span_16](end_span)
      fetchProducts(); [span_17](start_span)// Re-fetch products[span_17](end_span)
    } catch (error) {
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        ...
        fetchProducts(); [span_22](start_span)// Re-fetch products[span_22](end_span)
      } catch (error) {
        [span_23](start_span)console.error("Failed to delete product:", error);[span_23](end_span)
        [span_24](start_span)alert("Failed to delete product.");[span_24](end_span)
      }
    }
  };
  ...
  const handleCsvFileSelect = (event) => {
    ...
    reader.onload = (e) => {
      [span_25](start_span)const data = new Uint8Array(e.target.result);[span_25](end_span)
      [span_26](start_span)const workbook = XLSX.read(data, { type: "array" });[span_26](end_span)
      const sheetName = workbook.SheetNames[0];
      const json = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { header: 1 }); // Get raw data to extract headers
      ...
      setCsvData(json.slice(1)); // Rest of the rows are data

      // Initialize mapping with default (empty) values
      setFieldMapping({
        title: "",
        description: "",
        sku: "",
      ...
      setShowCsvMapping(true); // Show the mapping section
    };
  ...
  const handleConfirmBulkUpload = async () => {
    ...
    const productsToCreate = csvData.map((row, rowIndex) => {
          ...
          if (columnIndex !== -1) {
            ...
            if (sanityField === "price") {
              ...
              if (isNaN(value)) {
                  ...
                  return null; // Skip this product or handle error
              }
            } else if (sanityField === "sku") {
              value = String(value); // Ensure SKU is a string
            }
      ...
      return product;
    }).filter(product => product !== null); // Filter out any null products due to errors
    ...
    [span_27](start_span)const transaction = client.transaction();[span_27](end_span)
    productsToCreate.forEach((product) => {
      [span_28](start_span)transaction.create(product);[span_28](end_span)
    });

    try {
      [span_29](start_span)await transaction.commit();[span_29](end_span)
      [span_30](start_span)alert("Products uploaded successfully!");[span_30](end_span)
      setCsvFile(null);
      setCsvHeaders([]);
      setCsvData([]);
      setFieldMapping({
      ...
      [span_31](start_span)fetchProducts();[span_31](end_span)
    } catch (error) {
      [span_32](start_span)console.error("Bulk upload failed:", error);[span_32](end_span)
  ...
  const handleBulkPriceAdjustment = async () => {
    if (!priceAdjustmentValue) {
      [span_33](start_span)alert("Please enter a value for price adjustment.");[span_33](end_span)
      return;
    }
    ...
    [span_34](start_span)]{_id, price}`;[span_34](end_span)
    try {
      [span_35](start_span)const productsToAdjust = await client.fetch(query);[span_35](end_span)
      if (productsToAdjust.length === 0) {
        [span_36](start_span)alert("No products found matching your criteria for adjustment.");[span_36](end_span)
        return;
      }

      [span_37](start_span)const transaction = client.transaction();[span_37](end_span)
      productsToAdjust.forEach((product) => {
        ...
        [span_38](start_span)transaction.patch(product._id).set({ price: newPrice });[span_38](end_span)
      });
      [span_39](start_span)await transaction.commit();[span_39](end_span)
      alert("Bulk price adjustment applied!");
      fetchProducts(); [span_40](start_span)// Re-fetch products[span_40](end_span)
    } catch (error) {
      [span_41](start_span)console.error("Bulk price adjustment failed:", error);[span_41](end_span)
      [span_42](start_span)alert("Failed to apply bulk price adjustment.");[span_42](end_span)
    }
  };

  if (loading)
    [span_43](start_span)return <div className="text-center py-8 text-lg font-semibold text-gray-700">Loading products...</div>;[span_43](end_span)
  if (error)
    [span_44](start_span)return <div className="text-center text-red-600 py-8 text-lg font-semibold">Error: {error}</div>;[span_44](end_span)
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-xl">
        ...
        <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              id="title"
              name="title"
              [span_45](start_span)value={newProduct.title}[span_45](end_span)
              [span_46](start_span)onChange={handleNewProductChange}[span_46](end_span)
              required
          ...
          <div>
            ...
            <input
              ...
              id="sku"
              name="sku"
              [span_47](start_span)value={newProduct.sku}[span_47](end_span)
              [span_48](start_span)onChange={handleNewProductChange}[span_48](end_span)
              required
              className="mt-1 block w-full px-4 py-2 border border-red-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-base bg-white transition duration-200"
            />
          </div>
          <div className="md:col-span-2">
            ...
            <textarea
              id="description"
              name="description"
              [span_49](start_span)value={newProduct.description}[span_49](end_span)
              [span_50](start_span)onChange={handleNewProductChange}[span_50](end_span)
              rows="3"
              className="mt-1 block w-full px-4 py-2 border border-red-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-base bg-white transition duration-200"
            ></textarea>
          </div>
          <div>
            ...
            <input
              ...
              id="brand"
              name="brand"
              [span_51](start_span)value={newProduct.brand}[span_51](end_span)
          ...
          <div>
            ...
            <input
              ...
              id="price"
              name="price"
              [span_53](start_span)value={newProduct.price}[span_53](end_span)
              [span_54](start_span)onChange={handleNewProductChange}[span_54](end_span)
              required
              [span_55](start_span)min="0"[span_55](end_span)
              [span_56](start_span)step="0.01"[span_56](end_span)
              className="mt-1 block w-full px-4 py-2 border border-red-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-base bg-white transition duration-200"
            />
          </div>
          <div className="md:col-span-2">
          ...
          <div className="md:col-span-2 text-center">
            <button
              type="submit"
              [span_58](start_span)className="w-full md:w-1/2 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg text-lg transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"[span_58](end_span)
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="col-span-1">
            ...
            <div className="flex items-center">
              <input
                type="number"
                [span_59](start_span)value={priceAdjustmentValue}[span_59](end_span)
                [span_60](start_span)onChange={(e) => setPriceAdjustmentValue(parseFloat(e.target.value))}[span_60](end_span)
              ...
              />
              <select
                [span_61](start_span)value={priceAdjustmentType}[span_61](end_span)
                [span_62](start_span)onChange={(e) => setPriceAdjustmentType(e.target.value)}[span_62](end_span)
          ...
          <div>
            ...
            <input
              type="text"
              [span_63](start_span)value={priceAdjustmentBrand}[span_63](end_span)
              [span_64](start_span)onChange={(e) => setPriceAdjustmentBrand(e.target.value)}[span_64](end_span)
          ...
          <div className="md:col-span-2">
            ...
            <input
              type="text"
              [span_65](start_span)value={priceAdjustmentSearchTerm}[span_65](end_span)
              [span_66](start_span)onChange={(e) => setPriceAdjustmentSearchTerm(e.target.value)}[span_66](end_span)
          ...
          <div className="md:col-span-2 text-center">
            <button
              ...
              [span_67](start_span)className="w-full md:w-1/2 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg text-lg transition-all duration-300 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"[span_67](end_span)
            >
      ...
      <div className="p-8 border border-red-200 rounded-xl bg-red-50/20 shadow-lg">
        ...
        [span_68](start_span){products.length === 0 && ([span_68](end_span)
          <p className="text-gray-600 text-center py-4 text-lg">No products found. Add some above!</p>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          [span_69](start_span){products.map((product) => ([span_69](end_span)
            <div
              key={product._id}
              [span_70](start_span)className="relative border border-red-100 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden bg-white group"[span_70](end_span)
            >
              [span_71](start_span){product.imageUrl && ([span_71](end_span)
                <img
                  src={product.imageUrl}
                  alt={product.title}
                  [span_72](start_span)className="w-full h-40 object-cover object-center group-hover:scale-105 transition-transform duration-300"[span_72](end_span)
                />
              )}
              <div className="p-4">
                <h4 className="text-lg font-bold text-gray-900 truncate mb-1">
                  [span_73](start_span){product.title}[span_73](end_span)
                </h4>
                [span_74](start_span)<p className="text-sm text-gray-600">SKU: {product.sku}</p>[span_74](end_span)
                <p className="text-xl font-extrabold text-red-700 mt-3">
                  [span_75](start_span)SEK {product.price.toFixed(2)}[span_75](end_span)
                </p>
                <div className="flex justify-between mt-5 space-x-3">
                  <button
                    onClick={() => alert(`Edit functionality for ${product.title} would go here!`)}
                    [span_76](start_span)className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-md text-sm transition-colors duration-200 transform hover:scale-105"[span_76](end_span)
                  >
                    <i className="fas fa-edit mr-1"></i>Edit
                  </button>
                  <button
                    [span_77](start_span)onClick={() => handleDeleteProduct(product._id)}[span_77](end_span)
                    [span_78](start_span)className="flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-md text-sm transition-colors duration-200 transform hover:scale-105"[span_78](end_span)
                  >
