...
function ProductDetail() {
  ...
  if (error) return <div className="text-red-500 text-center py-10">{t('common.error')}</div>;
  if (!product) return <div>{t('productList.noProductsFound')}</div>;
    ...
    <div className="container mx-auto p-8 max-w-7xl">
      ...
      <div className="bg-white shadow-xl rounded-lg overflow-hidden md:flex">
        <div className="md:w-1/2">
          <img src={product.imageUrl || "https://placehold.co/400x300?text=BILD%20KOMMER%20INKOM%20KORT"} alt={product.title} className="w-full h-full object-cover"/>
        </div>
        <div className="md:w-1/2 p-8 flex flex-col justify-center">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">{product.title}</h1>
          <p className="text-sm text-gray-500 mb-4">{t('common.sku')}: {product.sku}</p>
          ...
          <div className="mb-6">
            {displayPrice.discounted ? (
              <>
                <p className="text-2xl text-gray-500 line-through">{t('productDetail.regularPrice')}: {displayPrice.original} kr</p>
                <p className="text-4xl font-extrabold text-green-600">{t('productDetail.yourPrice')}: {displayPrice.discounted} kr</p>
              </>
            ) : (
              <p className="text-3xl font-extrabold text-gray-800">{displayPrice.original} kr</p>
            )}
          </div>
          <button onClick={() => addToCart(product)} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg">
